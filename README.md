...
| --- | --- |
| `index.html` | 五页导航首页 |
| `自助取数.html`、`指标中心.html`、`数据血缘.html`、`平台导航.html`、`常用工具.html` | 独立页面入口，加载 app.html |
| `app.html` | 共享应用页面 |
...
- 保留五页导航及两个共享应用入口；更新应用时应同步 `app.html` 和 `传输库血缘与自助取数.html`。
...
页面顶部可切换「任务结束上报」与「心跳上报」。两空间使用独立目录、字段关系、表选择、搜索及页签状态；切换后保留各自当前浏览位置，浏览器记住所选空间。心跳空间开放数据血缘、平台导航及常用工具，不配置指标或取数模板。
...
平台导航通过共享应用中的 `platformConfig` 配置渲染，包含名称、描述、地址、图标与分类；卡片在新标签页打开平台。
