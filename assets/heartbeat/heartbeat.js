    ...
    function bindHeartbeat(root){
      $$('[data-hb-source]',root).forEach(b=>b.onclick=()=>{
        ...
        $('#infoDialogTitle').textContent=s.table+' · 线上 SQL';
        $('#infoDialogBody').innerHTML=`<p class="source-caption">${esc(s.file)}</p><code class="source-table-name">${esc(s.table)}</code><div class="source-sql-actions"><a class="source-link" href="${s.file.split('/').map(encodeURIComponent).join('/')}" download>下载本表 SQL</a><button type="button" class="btn small" id="copySourceSql" aria-live="polite">复制 SQL</button></div><pre class="production-sql numbered-sql"><code>${sqlToHtml(s.sql).split('\n').map((row,i)=>`<span class="source-code-line ${i+1===line?'source-highlight':''}"><i>${i+1}</i>${row||' '}</span>`).join('')}</code></pre>`;
        bindSourceSqlCopy(s.sql);$('#infoDialog').showModal();requestAnimationFrame(()=>$('.source-highlight',$('#infoDialogBody'))?.scrollIntoView({block:'center'}));
      });
    ...
    function renderHeartbeat(){
      ...
      const node=t=>`<button class="graph-node ${t.id===selected.id?'active':''}" style="--node:${typeColor[t.type]||'#64748b'};${search&&`${t.cn} ${t.name}`.toLowerCase().includes(search)?'box-shadow:0 0 0 4px #fbbf24;':''}" data-hb-table="${t.id}"><span class="tag">${t.type.toUpperCase()}</span><strong>${esc(t.name)}</strong><small>${esc(t.grain)} · ${t.frequency}</small></button>`;
    ...
    function renderHeartbeatFields(){
      ...
      $('#hbFieldDetail').innerHTML=`<div class="field-detail-heading"><div><h2>${esc(hbFieldName(s,f))}</h2><code>${esc(f)}</code></div><span class="tag">${esc(s.table)}</span></div><p class="source-caption">${esc(s.schema[f]?.comment||'SQL SELECT 输出列')} · 类型注释：${esc(s.schema[f]?.type||'未在 DDL 注释中声明')}</p><h3>当前层加工</h3>${sqlPanel(col.expression)}${hbSourceButton(s.id,col.line)}<h3>字段血缘 <small>从当前层向上游追溯</small></h3><div class="production-chain">${[...new Set(trace.nodes.map(n=>n.stage))].sort((a,b)=>['ADS','DWS','DWD','ODS'].indexOf(hbStages[a].layer)-['ADS','DWS','DWD','ODS'].indexOf(hbStages[b].layer)).map(id=>`<details class="production-step" ${id===s.id?'open':''}><summary><span>${esc(hbStages[id].table)}</span><small>${trace.nodes.filter(n=>n.stage===id).length} 项相关表达式</small></summary><code class="source-table-name">${esc(hbStages[id].table)}</code>${trace.nodes.filter(n=>n.stage===id).sort((a,b)=>a.line-b.line).map(n=>`<div class="production-expression"><b>${esc(n.field)}</b>${sqlPanel(n.expression)}${n.note?`<p class="source-caption">${esc(n.note)}</p>`:''}${hbSourceButton(id,n.line)}<p class="source-caption">输入：${n.refs.map(r=>esc(r.field)).join('、')||'固定值 / 常量'}</p></div>`).join('')}</details>`).join('')}</div><h3>原始来源</h3><div class="production-origin">${trace.external.map(x=>`<code>${esc(x.table)}.${esc(x.field)}</code>`).join('<br>')||'由 SQL 常量或系统函数生成，无直接原始字段。'}</div><details class="field-context"><summary>过滤、去重与关联条件</summary><p>${esc(s.note)}</p><p>上方链路展示字段值的直接依赖；过滤和分组条件可在各层完整 SQL 中查看。</p>${[...new Set(trace.nodes.map(n=>n.stage))].map(id=>hbSourceButton(id,1,hbStages[id].table)).join('')}</details>`;
    ...
    function initHeartbeat(){
      $('#hbFieldTable').innerHTML=heartbeat.stages.slice().sort((a,b)=>a.direction.localeCompare(b.direction)||['ODS','DWD','DWS','ADS'].indexOf(a.layer)-['ODS','DWD','DWS','ADS'].indexOf(b.layer)).map(s=>`<option value="${s.id}">${esc(s.table)}</option>`).join('');
